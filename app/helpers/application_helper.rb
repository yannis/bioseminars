...
module ApplicationHelper
  ...
  def flash_in_js
    "$.gritter.add({title: 'warning', class_name: 'warning',  text: '#{flash[:alert]}'});".html_safe unless flash[:alert].blank?
    "$.gritter.add({title: 'notice', class_name: 'notice',  text: '#{flash[:notice]}'});".html_safe unless flash[:notice].blank?
  end
  ...
  def load_publications_for(*seminars)
    ...
    for seminar in seminars
      js += "$.ajax({url: '#{load_publications_seminar_path(seminar)}'});".html_safe
    end
    return js
  end
