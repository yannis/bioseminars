...
class Seminar < ActiveRecord::Base
  ...
  def next_seminar
    Seminar.where("seminars.start_on > ?", self.start_on).order('seminars.start_on ASC').first
  end
  def next_seminar_id
    next_seminar.id if next_seminar
  end

  def previous_seminar
    Seminar.where("seminars.start_on < ?", self.start_on).order('seminars.start_on DESC').first
  end

  def previous_seminar_id
    previous_seminar.id if previous_seminar
  end
